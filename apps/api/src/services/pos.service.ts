...
export async function createPosSale(sellerId: string, lines: PosSaleLine[]) {
  if (lines.length === 0) throw new ApiError(400, "At least one item is required");
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      for (const line of lines) {
        ...
        if (!product) throw new ApiError(404, "Product not found in your catalog");
