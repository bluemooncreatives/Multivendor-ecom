...
export async function createInstamojoRequest(orderId: string) {
  const order = await getPendingOrderOrThrow(orderId);
  const creds = await getGatewayCredentials<InstamojoCreds>("instamojo");
  const addr = order.addressSnapshot as Record<string, string>;

  const { data } = await axios.post(
    `${baseUrl(creds.sandbox)}/payment-requests/`,
    new URLSearchParams({
  ...
  if (!data.success) throw new ApiError(502, "Instamojo payment request failed");
  await recordPendingPayment(orderId, "instamojo", order.grandTotal, order.currency, data.payment_request.id);
  return { redirectUrl: data.payment_request.longurl as string };
}
...
export async function confirmInstamojoPayment(paymentRequestId: string, paymentId: string, orderId: string) {
  const creds = await getGatewayCredentials<InstamojoCreds>("instamojo");
