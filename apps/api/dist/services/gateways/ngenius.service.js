...
async function getAccessToken(creds) {
    const { data } = await axios.post(`${baseUrl(creds.sandbox)}/identity/auth/access-token`, {}, {
        headers: {
...
export async function createNgeniusOrder(orderId) {
    const order = await getPendingOrderOrThrow(orderId);
    const creds = await getGatewayCredentials("ngenius");
    const token = await getAccessToken(creds);
    const { data } = await axios.post(`${baseUrl(creds.sandbox)}/transactions/outlets/${creds.outletId}/orders`, {
        action: "SALE",
    ...
    if (!paymentUrl)
        throw new ApiError(502, "N-Genius order creation did not return a payment link");
    await recordPendingPayment(orderId, "ngenius", order.grandTotal, order.currency, data.reference);
    return { redirectUrl: paymentUrl };
}
export async function confirmNgeniusOrder(reference, orderId) {
    const creds = await getGatewayCredentials("ngenius");
    const token = await getAccessToken(creds);
...
//# sourceMappingURL=ngenius.service.js.map