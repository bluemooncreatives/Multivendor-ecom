...
export async function createSslcommerzSession(orderId) {
    const order = await getPendingOrderOrThrow(orderId);
    const creds = await getGatewayCredentials("sslcommerz");
    const addr = order.addressSnapshot;
    const { data } = await axios.post(`${baseUrl(creds.sandbox)}/gwprocess/v4/api.php`, new URLSearchParams({
    ...
    if (data.status !== "SUCCESS")
        throw new ApiError(502, "SSLCommerz session initiation failed");
...
export async function validateSslcommerzPayment(valId, orderId) {
    const creds = await getGatewayCredentials("sslcommerz");
