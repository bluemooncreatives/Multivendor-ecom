...
export async function createPaystackTransaction(orderId) {
    ...
    if (!data.status)
        throw new ApiError(502, "PayStack transaction initialization failed");
...
export async function verifyPaystackTransaction(reference, orderId) {
    const creds = await getGatewayCredentials("paystack");
...
export async function verifyPaystackWebhookSignature(rawBody, signature) {
    const creds = await getGatewayCredentials("paystack").catch(() => null);
    if (!creds || !signature)
        return false;
