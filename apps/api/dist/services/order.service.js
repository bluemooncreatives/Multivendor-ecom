...
export async function cancelOrder(orderId, requester) {
    ...
    try {
        ...
        await session.withTransaction(async () => {
            ...
            for (const detail of order.details) {
                for (const item of detail.items) {
                    if (wasConfirmed) {
                        ...
                        await Product.updateOne({ _id: item.productId, "variants.sku": item.variantSku }, { $inc: { "variants.$[v].stock": item.quantity } }, { session, arrayFilters: [{ "v.sku": item.variantSku }] });
                    }
                    else {
                        // Still just a reservation — release the hold.
                        await Product.updateOne({ _id: item.productId, "variants.sku": item.variantSku }, { $inc: { "variants.$[v].reserved": -item.quantity } }, { session, arrayFilters: [{ "v.sku": item.variantSku }] });
                    }
            ...
            if (order.paymentMethod === "wallet" && order.paymentStatus === "paid" && order.userId) {
                const wallet = await Wallet.findOneAndUpdate({ userId: order.userId }, { $inc: { balance: order.grandTotal } }, { session, new: true });
