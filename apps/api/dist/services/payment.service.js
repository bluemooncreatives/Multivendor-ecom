...
import { confirmReservation, releaseReservation } from "./inventory.service.js";
...
const razorpay = env.RAZORPAY_KEY_ID && env.RAZORPAY_KEY_SECRET
    ...
    : null;
function stockLinesFromOrder(order) {
    return order.details.flatMap((detail) => detail.items.map((item) => ({
...
export async function settleOrderPayment(orderId, method, providerRef) {
    ...
    try {
        await session.withTransaction(async () => {
            ...
            if (!order)
                throw new ApiError(404, "Order not found");
            if (order.paymentStatus === "paid")
                return; // already settled — webhook replay is a no-op
            ...
            await Payment.updateOne({ orderId: order._id, method }, { status: "paid", providerRef }, { session });
        });
    }
    finally {
...
export async function failOrderPayment(orderId, method) {
    const session = await mongoose.startSession();
    try {
        await session.withTransaction(async () => {
            const order = await Order.findById(orderId).session(session);
...
export async function recordPaymentEventOnce(provider, eventId, payload) {
    try {
