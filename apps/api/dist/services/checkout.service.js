...
const SYNCHRONOUS_METHODS = ["cod", "wallet"];
export async function createOrder(input) {
    const existing = await Order.findOne({ idempotencyKey: input.idempotencyKey });
    if (existing)
        return existing; // replayed request — return the already-created order, don't double-charge
    ...
    if (input.addressId) {
        if (!input.owner.userId)
            throw new ApiError(400, "Saved addresses require an account");
    ...
    try {
        ...
        await session.withTransaction(async () => {
            ...
            for (const cartItem of cart.items) {
                ...
                if (!variant)
                    throw new ApiError(409, "One of the selected variants is no longer available");
            ...
            if (input.paymentMethod === "wallet") {
                if (!input.owner.userId)
                    throw new ApiError(400, "Wallet payment requires an account");
                const debited = await Wallet.findOneAndUpdate({ userId: input.owner.userId, balance: { $gte: grandTotal } }, { $inc: { balance: -grandTotal } }, { session, new: true });
                if (!debited)
