...
export async function confirmReservation(lines, orderId, session) {
    for (const line of lines) {
        await Product.updateOne({ _id: line.productId, "variants.sku": line.variantSku }, { $inc: { "variants.$[v].reserved": -line.quantity, "variants.$[v].stock": -line.quantity } }, { session, arrayFilters: [{ "v.sku": line.variantSku }] });
        await InventoryMovement.create([{ productId: line.productId, variantSku: line.variantSku, type: "confirm", quantity: line.quantity, orderId }], { session });
    }
}
// Releases a reservation without touching sellable stock (order cancelled before payment).
export async function releaseReservation(lines, orderId, session) {
    for (const line of lines) {
        await Product.updateOne({ _id: line.productId, "variants.sku": line.variantSku }, { $inc: { "variants.$[v].reserved": -line.quantity } }, { session, arrayFilters: [{ "v.sku": line.variantSku }] });
        await InventoryMovement.create([{ productId: line.productId, variantSku: line.variantSku, type: "release", quantity: line.quantity, orderId }], { session });
    }
}
