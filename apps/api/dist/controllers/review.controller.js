...
export async function createReviewHandler(req, res) {
    ...
    const purchased = order.details.some((detail) => detail.status === "delivered" && detail.items.some((item) => String(item.productId) === productId));
    if (!purchased)
        throw new ApiError(403, "You can only review products from delivered orders");
    const review = await Review.findOneAndUpdate({ userId: req.user.id, orderId, productId }, { userId: req.user.id, orderId, productId, rating, comment, images }, { upsert: true, new: true, setDefaultsOnInsert: true });
    const stats = await Review.aggregate([
        { $match: { productId: review.productId, approved: true } },
        { $group: { _id: null, avg: { $avg: "$rating" }, count: { $sum: 1 } } },
    ]);
    await Product.updateOne({ _id: productId }, { ratingAverage: stats[0]?.avg ?? rating, ratingCount: stats[0]?.count ?? 1 });
    res.status(201).json(review);
}
export async function listProductReviewsHandler(req, res) {
    const reviews = await Review.find({ productId: req.params.productId, approved: true })
        .populate("userId", "name avatarUrl")
        .sort({ createdAt: -1 });
    res.json({ items: reviews });
}
export async function listSellerReviewsHandler(req, res) {
...
export async function replyReviewHandler(req, res) {
    const review = await Review.findById(req.params.id);
    if (!review)
        throw new ApiError(404, "Review not found");
    const product = await Product.findOne({ _id: review.productId, sellerId: req.user.id });
    if (!product)
        throw new ApiError(403, "You do not own the product for this review");
    review.sellerReply = req.body.sellerReply;
    await review.save();
    res.json(review);
}
export async function listAdminReviewsHandler(_req, res) {
...
export async function moderateReviewHandler(req, res) {
