...
export async function deleteProductHandler(req, res) {
    const deleted = await Product.findOneAndUpdate({ _id: req.params.id, sellerId: req.user.id }, { published: false });
    if (!deleted)
        throw new ApiError(404, "Product not found");
    res.status(204).send();
}
...
export async function cloneProductHandler(req, res) {
...
export async function listSellerProductsHandler(req, res) {
