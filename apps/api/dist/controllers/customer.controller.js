...
export async function addWishlistHandler(req, res) {
    const item = await Wishlist.findOneAndUpdate({ userId: req.user.id, productId: req.body.productId }, { userId: req.user.id, productId: req.body.productId }, { upsert: true, new: true });
...
export async function listMyTicketsHandler(req, res) {
...
export async function replyTicketHandler(req, res) {
    ...
    ticket.replies.push({ authorId: req.user.id, message: req.body.message });
