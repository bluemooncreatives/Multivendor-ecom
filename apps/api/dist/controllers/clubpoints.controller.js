...
export async function getMyClubPointsHandler(req, res) {
    const user = await User.findById(req.user.id);
    res.json({ points: user?.clubPoints ?? 0 });
}
export async function listMyClubPointHistoryHandler(req, res) {
    const items = await ClubPointTransaction.find({ userId: req.user.id }).sort({ createdAt: -1 }).limit(200);
    res.json({ items });
}
//# sourceMappingURL=clubpoints.controller.js.map