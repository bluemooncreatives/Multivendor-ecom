...
async function seed() {
    ...
    // --- Settings ---
    await GeneralSetting.findOneAndUpdate({ key: "general" }, { key: "general", appName: "PHPStore - Be Vocal for Local - Digital India", supportEmail: "support@v4local.in" }, { upsert: true });
    await SeoSetting.findOneAndUpdate({ key: "seo" }, { key: "seo", metaTitle: "PHPStore — Be Vocal for Local", googleAnalyticsId: "G-T7899BYWWM" }, { upsert: true });
    await BusinessSetting.findOneAndUpdate({ key: "business" }, { key: "business", taxPercent: 5, commissionPercent: 10, shippingMode: "flat", flatShippingCost: 49 }, { upsert: true });
    ...
    const admin = await User.findOneAndUpdate({ email: "admin@example.com" }, { name: "Admin", email: "admin@example.com", passwordHash, role: "admin", emailVerifiedAt: new Date() }, { upsert: true, new: true });
    const staff = await User.findOneAndUpdate({ email: "staff@example.com" }, {
        name: "Staff Member",
    ...
    const seller = await User.findOneAndUpdate({ email: "seller@example.com" }, { name: "Demo Seller", email: "seller@example.com", passwordHash, role: "seller", emailVerifiedAt: new Date() }, { upsert: true, new: true });
    const customer = await User.findOneAndUpdate({ email: "customer@example.com" }, { name: "Demo Customer", email: "customer@example.com", passwordHash, role: "customer", emailVerifiedAt: new Date() }, { upsert: true, new: true });
    ...
    await Shop.findOneAndUpdate({ sellerId: seller._id }, {
        sellerId: seller._id,
    ...
    // --- Marketing ---
    await Coupon.findOneAndUpdate({ code: "WELCOME10" }, { code: "WELCOME10", type: "percent", value: 10, minOrderValue: 500, maxDiscount: 300, usageLimitPerUser: 1, active: true }, { upsert: true });
